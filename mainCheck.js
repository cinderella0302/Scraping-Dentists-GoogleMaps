...
async function getRequestForRegion(zipcode, cookie, code) {
    ...
    try{
        ...
        //     console.log('---- no matching dentist');
        // } else {
...
for(let i = 285; i < zipcodeData.length; i++) {
    console.log('-------------------------------zipcode: ', zipcodeData[i], ' ----index: ', i);
    const existingDoc = await Regions.findOne({zipcode: zipcodeData[i]});
    if(existingDoc){
        console.log('---------- already exist zipcode: ', zipcodeData[i]);
    } else {
